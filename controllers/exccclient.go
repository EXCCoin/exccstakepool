...
type validateAddressResponse struct {
	addrInfo *exccjson.ValidateAddressWalletResult
	err      error
}
...
type createMultisigResponse struct {
	multisigInfo *exccjson.CreateMultiSigResult
	err          error
}
...
type ticketsForAddressResponse struct {
	tickets *exccjson.TicketsForAddressResult
	err     error
}
...
type getStakeInfoResponse struct {
	stakeInfo *exccjson.GetStakeInfoResult
	err       error
}
...
type connectedResponse struct {
	walletInfo []*exccjson.WalletInfoResult
	err        error
}
...
type stakePoolUserInfoResponse struct {
	userInfo *exccjson.StakePoolUserInfoResult
	err      error
}

// stakePoolUserInfoMsg
type stakePoolUserInfoMsg struct {
	userAddr          exccutil.Address
...
func (w *walletSvrManager) executeInSequence(fn functionName, msg interface{}) interface{} {
	...
	case validateAddressFn:
		...
		vawrs := make([]*exccjson.ValidateAddressWalletResult, w.serversLen)
		connectCount := 0
	...
	case createMultisigFn:
		...
		cmsrs := make([]*exccjson.CreateMultiSigResult, w.serversLen)
		connectCount := 0
		for i, s := range w.servers {
			if w.servers[i] == nil {
				continue
	...
	case ticketsForAddressFn:
		...
		tfars := make([]*exccjson.TicketsForAddressResult, w.serversLen)
		connectCount := 0
		for i, s := range w.servers {
			if w.servers[i] == nil {
				continue
	...
	case getTxOutFn:
		...
		gtors := make([]*exccjson.GetTxOutResult, w.serversLen)
		connectCount := 0
		for i, s := range w.servers {
			if w.servers[i] == nil {
				continue
	...
	case getStakeInfoFn:
		...
		gsirs := make([]*exccjson.GetStakeInfoResult, w.serversLen)
		connectCount := 0
		for i, s := range w.servers {
			if w.servers[i] == nil {
				continue
	...
	case connectedFn:
		...
		resp.err = nil
		wirs := make([]*exccjson.WalletInfoResult, w.serversLen)
	...
	case stakePoolUserInfoFn:
		...
		spuirs := make([]*exccjson.StakePoolUserInfoResult, w.serversLen)
...
func (w *walletSvrManager) connected() ([]*exccjson.WalletInfoResult, error) {
...
func (w *walletSvrManager) syncTickets(spuirs []*exccjson.StakePoolUserInfoResult) error {
...
// call is made.
func (w *walletSvrManager) checkForSyncness(spuirs []*exccjson.StakePoolUserInfoResult) bool {
	for i := 0; i < len(spuirs); i++ {
		if spuirs[i] == nil {
			continue
		}
...
func (w *walletSvrManager) ValidateAddress(addr exccutil.Address) (*exccjson.ValidateAddressWalletResult, error) {
...
// This should return equivalent results from all wallet RPCs. If this
// encounters a failure, it should be considered fatal.
func (w *walletSvrManager) CreateMultisig(nreq int, addrs []exccutil.Address) (*exccjson.CreateMultiSigResult, error) {
	// Assert that all servers are online.
	_, err := w.connected()
	if err != nil {
		return nil, connectionError(err)
...
func (w *walletSvrManager) TicketsForAddress(address exccutil.Address) (*exccjson.TicketsForAddressResult, error) {
...
// from this function should NOT cause fatal errors on the web server like the
// other RPC client calls.
func (w *walletSvrManager) GetTxOut(hash *chainhash.Hash, idx uint32) (*exccjson.GetTxOutResult, error) {
...
// from this function should NOT cause fatal errors on the web server like the
// other RPC client calls.
func (w *walletSvrManager) StakePoolUserInfo(userAddr exccutil.Address, takeFirstResponse bool) (*exccjson.StakePoolUserInfoResult, error) {
...
// from this function should NOT cause fatal errors on the web server like the
// other RPC client calls.
func (w *walletSvrManager) getStakeInfo() (*exccjson.GetStakeInfoResult, error) {
...
type getTicketsCacheData struct {
	res   *exccjson.TicketsForAddressResult
	timer time.Time
}

// NewGetTicketsCacheData is a contructor for getTicketsCacheData that sets the
// last get time to now.
func NewGetTicketsCacheData(tfar *exccjson.TicketsForAddressResult) *getTicketsCacheData {
...
type walletSvrManager struct {
	...
	cachedStakeInfo      *exccjson.GetStakeInfoResult
...
func (w *walletSvrManager) GetUnspentUserTickets(userMultiSigAddress exccutil.Address) ([]*chainhash.Hash, error) {
	// live tickets only
...
func (w *walletSvrManager) WalletStatus() ([]*exccjson.WalletInfoResult, error) {
	return w.connected()
}
...
// missing.
func (w *walletSvrManager) fetchTransaction(txHash *chainhash.Hash) (*exccutil.Tx,
	error) {
	var tx *exccutil.Tx
	var err error
	for i := range w.servers {
...
// newWalletSvrManager returns a new excc wallet server manager.
